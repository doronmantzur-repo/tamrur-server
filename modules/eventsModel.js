...
async function update_event(id, updates) {
  const query =
    'UPDATE events SET name = COALESCE(:name, name), status = COALESCE(:status, status), closure_at = COALESCE(:closure_at, closure_at), type = COALESCE(:type, type), location = COALESCE(ST_SetSRID(ST_GeomFromGeoJSON(:location), 4326)::geography, location), "aerial-evac" = COALESCE(:aerialEvac, "aerial-evac"), gathering_status = COALESCE(:gatheringStatus, gathering_status) WHERE id = :id RETURNING *;';
  try {
    const [result] = await sequelize.query(query, {
      replacements: {
        id,
